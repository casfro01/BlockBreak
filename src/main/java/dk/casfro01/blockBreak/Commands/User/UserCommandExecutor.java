...
import dk.casfro01.blockBreak.Commands.CommandBuilder;
import dk.casfro01.blockBreak.Commands.CommandFactory;
import dk.casfro01.blockBreak.Commands.CommandInvoker;
...
public class UserCommandExecutor implements org.bukkit.command.CommandExecutor{
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if (!commandSender.hasPermission("blocks.block")){
            commandSender.sendMessage(ChatColor.RED + "Du har ikke adgang til systemet.");
            return false;
        }
        if (strings.length < 1){
            invoker.executeCommand(new CommandBuilder().setCommand("info", CommandFactory.CommandType.USER).setPlayer(commandSender).getCommand());
            return true;
        }
